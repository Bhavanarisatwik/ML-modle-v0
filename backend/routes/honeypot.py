...
async def receive_honeypot_log(
    ...
    try:
        ...
        if AUTH_ENABLED:
            ...
            node = await db_service.get_node_by_id(log.node_id)
            if not node:
                raise HTTPException(
                    status_code=404,
            ...
            if node.get("api_key") != x_node_key:
                logger.warning(f"Invalid API key attempt for node {log.node_id}")
                raise HTTPException(
                    status_code=403,
            ...
            if node["status"] != "active":
                raise HTTPException(
                    status_code=403,
                    detail=f"Node {log.node_id} is inactive"
                )
            ...
            await db_service.update_node_last_seen(
                log.node_id,
                node_service.update_last_seen(log.node_id)
            )
        
        # Step 2: Get ML prediction
        ml_prediction = await ml_service.predict_attack(log.dict())
